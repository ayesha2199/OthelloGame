...
//If atleast one direction is returned true, the move is legal 
if (direction >1)
validMove=true;
else 
validMove=false;
...
//Finds the difference between the flips scored in a move and the highest number of pieces lost to opponent
int surplus(char board[26][26], int n, int row, int col,  char compColour, char AIcolour)
{
	int surplus =0;
    int scoreGain;
    int scoreLost=0;
    ...
    for (k=0; k<=n; k++){
        for (m=0; m<=n; m++){
            tempBoard[k][m]=board[k][m];
        }
    }
    
      int i, j;
     if(LegalMove(board, n, row, col, compColour)==true && board[row][col]=='U')
     {			// makes temporary board that emulates the board to look a move ahead 
         tempBoard[row][col]=compColour;
         flipPieces(tempBoard, n, row, col, compColour);
         scoreGain = scoreOfMove(tempBoard, n, row, col, compColour);
         
     }
     
       for (i = 0; j <= n; i++) {
        for (j = 0; j <= n; j++) {
            if (LegalMove(tempBoard, n, i, j, AIcolour)==true && tempBoard[i][j]=='U')
            {///find difference using the maximum score for opponent 
                    if (scoreLost<scoreOfMove(tempBoard,n,i, j, AIcolour))            
                scoreLost= scoreOfMove(tempBoard,n,i, j, AIcolour);
            }
        }
       }
       surplus = scoreGain-scoreLost;
       
       return surplus;
       
}

//prints the move for the comupter user using various strategies 
void printMoveForComputer(char board[26][26], int n, char colour, char opposite, int e) {
    int i, j, row, col;
    int k = 0;
    int score = 0;
	//checks each element of the board for a possible move 
    for (i = 0; i <= n; i++) {
        for (j = 0; j <= n; j++) {
			//checks legal move first 
            if (board[i][j] == 'U' && LegalMove(board, n, i, j, colour) == true) {
              k=0;
			  //prioritizes corners as #1 
                    if ((i == 0 && j == 0) ) {
                    row = i;
                    col = j;
                    k=1;
             ...
             //places pieces one away from corners to increase possibility of achieving a corner 
                if (((i==0 && j==2) || (i==0 && j==n-2) ||  (i==2 && j==0) || (i==2 && j==n) || (i==n && j==2) || (i==n && j==n-2)
                            || (i==n-2 && j==0) || (i==n-2 && j==n)))
                            {
                                row = i;
                                col = j;
                      ...
                      //places at walls only if surplus is greater than 2           
                    else if (((i==2 && j==2) ||(i==2 && j==n-2) || (i==n-2 && j==2) ||  (i==n-2 && j==n-2))&& surplus(board, n, i, j, colour, opposite)>2)
                  {
                                row = i;
                                col = j;
               ...
               //takes walls   
                   ...
                   else if (score < scoreOfMove(board, n, i, j, colour)) {
...
k=1;     
    ...
    if (LegalMove(board, n, row, col, colour) == true && board[row][col] == 'U') {
...
//prints the users moves using what is entered
...
//promts user to enter dimension and board configuration and assigns corresponding values 
